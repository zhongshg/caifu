...
<%    int CurrentPage = RequestUtil.getInt(request, "page");
    ...
    if (null != subscriberid && !"".equals(subscriberid)) {
        Map<String, String> subscriber = new SubscriberDAO().getById(subscriberid);
...
//                    DaoFactory.getFundsDao().del(objid);
                    ...
                    for (String fno : objid) {
                        ...
                        Map<String, String> map = subscriberDAO.getByOpenid(df.getFieldValue("wxsid"), df.getFieldValue("UserId"));
                        ...
                        } else if (!subscriberDAO.updateTx(df.getFieldValue("UserId"), df.getFieldValue("wxsid"), df.getFloat("F_Price"))) {
        ...
        if ("export".equals(act)) {
            ...
            if ("1".equals(sign)) {
                ...
                list = (ArrayList) DaoFactory.getFundsDao().getList(0, Type, FNo, UserId, null, 0, wxqueryid, -1, -1);
            }
            ...
            for (Iterator iter = list.iterator(); iter.hasNext(); i++) {
                ...
                wxtemp.put("id", df.getFieldValue("wxsid"));
                ...
                object[i][7] = wxtemp.get("name");
            }
            map.put("提现表", object);
            boolean flag = ExportExcel.createExcel(path, map);
            if (flag) {
                DaoFactory.getExportlogDaoImplJDBC().add("提现表", webpath, WxMenuUtils.format.format(new java.util.Date(nonce)), "0", Integer.parseInt(((Map<String, String>) session.getAttribute("users")).get("id")));
...
<html xmlns="http://www.w3.org/1999/xhtml">
    ...
    <script>
        function goUrl(frm)
        {
            var gourl = "?Type=<%=Type%>&FNo=<%=FNo%>&wxqueryid=<%=wxqueryid%>&";
    ...
    <body>
        ...
        <form action="?Type=<%=Type%>&FNo=<%=FNo%>&UserId=<%=UserId%>&act=bat" method="post">
            <table width="98%" height="25" border="0" align="center" cellpadding="2" cellspacing="1" bgcolor="#A3B2CC">
                ...
                <tr>
                    ...
                    <td width="10%" align="center" bgcolor="#3872b2" class="style2">vipid</td>
                ...
                <%
                    TotalNum = DaoFactory.getFundsDao().getTotalNum(0, Type, FNo, UserId, null, 0, wxqueryid, FromDate, ToDate);
                    PageNum = (TotalNum - 1 + PageSize) / PageSize;
                    ArrayList list = (ArrayList) DaoFactory.getFundsDao().getList(0, Type, FNo, UserId, null, 0, wxqueryid, FromDate, ToDate, CurrentPage, PageSize);
                    float price = 0.0f;
                    for (Iterator iter = list.iterator(); iter.hasNext();) {
                        ...
                        map.put("id", df.getFieldValue("wxsid"));
                        map = new WxsDAO().getById(map);

                        Map<String, String> subscriber = new SubscriberDAO().getByOpenid(df.getFieldValue("UserId"));
                %>
                <tr>
                    <td bgcolor="#FDFDFD">
        ...
        <form>
            <div align="center"><%=CurrentPage%>/<%=PageNum%>&nbsp;&nbsp;共:<%=TotalNum%>&nbsp;&nbsp;
                <%if (CurrentPage > 1) {%>
                <a href="?Type=<%=Type%>&FNo=<%=FNo%>&UserId=<%=UserId%>&wxqueryid=<%=wxqueryid%>&page=<%=CurrentPage - 1%>">上一页</a>&nbsp;&nbsp;
                <%} else {%>
                上一页&nbsp;&nbsp;
                <%}%>
                ...
                下一页
                <%} else {%>
                <a href="?Type=<%=Type%>&FNo=<%=FNo%>&UserId=<%=UserId%>&wxqueryid=<%=wxqueryid%>&page=<%=CurrentPage + 1%>">下一页</a>
                <%}%>
                跳转：
        ...
        function exportExcel(sign) {
            ...
            if ("1" == sign && value == "") {
                ...
                if (confirm("确认导出？")) {
                    window.location.href = "?act=export&Type=<%=Type%>&FNo=<%=FNo%>&page=<%=CurrentPage%>&value=" + value + "&sign=" + sign;
                }
            }
        }
        </script>
