    ...
    <body>
                ...
                <tr>
                    <td height="32">
                        微信：<select name="wxqueryid">
                            ...
                            <option value="0" <%="0".equals(wxqueryid) ? "selected='selected'" : ""%>>管理员</option>
                            <%
        ...
        <form action="?act=bat" method="post">
            <table width="98%" height="25" border="0" align="center" cellpadding="2" cellspacing="1" bgcolor="#A3B2CC">
                ...
                <tr>
                    ...
                    <td width="7%" bgcolor="#3872b2"><div align="center" class="style2">微信</div></td>
                </tr>
                <%
                    TotalNum = DaoFactory.getExportlogDaoImplJDBC().getTotalNum(wxqueryid);

                    PageNum = (TotalNum - 1 + PageSize) / PageSize;
                    ArrayList list = (ArrayList) DaoFactory.getExportlogDaoImplJDBC().getList(wxqueryid, CurrentPage, PageSize);
                    float price = 0.0f;
                ...
                <tr>
                    ...
                    <td align="center" bgcolor="#FDFDFD"><a href="<%=df.getFieldValue("path")%>"><%=df.getFieldValue("name")%></a></td>
                    ...
                    <td bgcolor="#FDFDFD"><div align="center"><%="0".equals(df.getFieldValue("wxsid")) ? "管理员" : map.get("name")%></div></td>
                </tr>
                <%
                    }
                %>
        ...
        <form>
            <div align="center"><%=CurrentPage%>/<%=PageNum%>&nbsp;&nbsp;共:<%=TotalNum%>&nbsp;&nbsp;
                <%if (CurrentPage > 1) {%>
                <a href="?page=<%=CurrentPage - 1%>&wxqueryid=<%=wxqueryid%>">上一页</a>&nbsp;&nbsp;
                <%} else {%>
                上一页&nbsp;&nbsp;
                <%}%>
                ...
                下一页
                <%} else {%>
                <a href="?page=<%=CurrentPage + 1%>&wxqueryid=<%=wxqueryid%>">下一页</a>
                <%}%>
                跳转：
