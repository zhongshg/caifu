...
<html xmlns="http://www.w3.org/1999/xhtml">
    ...
    <script>
        function goUrl(frm)
        {
            var gourl = "?cid=<%=cid%>&key=<%=(key == null ? "" : key)%>&sts=<%=sts%>&rem=<%=rem%>&isnew=<%=isnew%>&wxqueryid=<%=wxqueryid%>&";
    ...
    <body>
                ...
                <tr>
                    <td height="32">
                        ...
                        分类：<select name="cid">
                            ...
                            <%
                                Collection<DataField> cList = DaoFactory.getCategoryDAO().getListByParentid(0);
        ...
        <form action="?cid=<%=cid%>&act=bat" method="post">
            <table width="98%" height="25" border="0" align="center" cellpadding="2" cellspacing="1" bgcolor="#A3B2CC">
                ...
                <%
                    ...
                    TotalNum = DaoFactory.getProductDAO().getTotalCount(null, wxqueryid, null, 0, 0, cid, 0, 0, 0, 0, 0, null, null, null, null, null, key, null, 0, null, null, 0, 0, 0, null, null, null, 1, 0, 0, null, null, null, null, sts, isnew, 0, null, 0, 0, null, null, 0, 0);
                    PageNum = (TotalNum - 1 + PageSize) / PageSize;
                    ArrayList list = (ArrayList) DaoFactory.getProductDAO().getList(null, wxqueryid, null, 0, 0, cid, 0, 0, 0, 0, 0, null, null, null, null, null, key, null, 0, null, null, 0, 0, 0, null, null, null, 1, 0, 0, null, null, null, null, sts, isnew, 0, 0, 0, null, 0, 0, null, null, 0, 0, "id desc", CurrentPage, PageSize);
        ...
        <form>
            <div align="center"><%=CurrentPage%>/<%=PageNum%>&nbsp;&nbsp;共:<%=TotalNum%>&nbsp;&nbsp;
                <%if (CurrentPage > 1) {%>
                <a href="?cid=<%=cid%>&key=<%=(key == null ? "" : key)%>&sts=<%=sts%>&rem=<%=rem%>&isnew=<%=isnew%>&wxqueryid=<%=wxqueryid%>&page=<%=CurrentPage - 1%>">上一页</a>&nbsp;&nbsp;
                <%} else {%>
                上一页&nbsp;&nbsp;
                <%}%>
                ...
                下一页
                <%} else {%>
                <a href="?cid=<%=cid%>&key=<%=(key == null ? "" : key)%>&sts=<%=sts%>&rem=<%=rem%>&isnew=<%=isnew%>&wxqueryid=<%=wxqueryid%>&page=<%=CurrentPage + 1%>">下一页</a>
                <%}%>
                跳转：
