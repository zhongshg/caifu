...
function AddChildAndToggle(item){
	...
	if(xmlhttp){	
		...
		var ReturnValue=xmlhttp.open("POST", "matter_getchilds.jsp?classid="+item,true);
		xmlhttp.send("");
	}
	Toggle(item);
}
...
function CreateTreeItem( parent, img1FileName, img2FileName,nodeId, nodeName, url, target,isManage)
{
	...
	a.setAttribute( "href","matter_add.jsp?parentid="+uniqueId);
	a.setAttribute( "title","add matter");
	cell.appendChild( a );
	...
	a.setAttribute( "href","matter_del.jsp?id="+uniqueId);
	a.setAttribute( "title","remove matter");
	cell.appendChild( a );
	text=document.createTextNode(" -");
