...
function AddChildAndToggle(item){
	...
	if(xmlhttp){	
		...
		var ReturnValue=xmlhttp.open("POST", "washing_getchilds.jsp?classid="+item,true);
		xmlhttp.send("");
	}
	Toggle(item);
}
...
function CreateTreeItem( parent, img1FileName, img2FileName,nodeId, nodeName, url, target,isManage)
{
	...
	a.setAttribute( "href","washing_add.jsp?parentid="+uniqueId);
	a.setAttribute( "title","add washing");
	cell.appendChild( a );
	...
	a.setAttribute( "href","washing_del.jsp?id="+uniqueId);
	a.setAttribute( "title","remove washing");
	cell.appendChild( a );
	text=document.createTextNode(" -");
