...
function AddChildAndToggle(item){
	...
	if(xmlhttp){	
		...
		var ReturnValue=xmlhttp.open("POST", "service_getchilds.jsp?classid="+item,true);
		xmlhttp.send("");
	}
	Toggle(item);
}
...
function CreateTreeItem( parent, img1FileName, img2FileName,nodeId, nodeName, url, target,isManage)
{
	...
	if( url != null )
	{
		...
		text=document.createTextNode(uniqueId+nodeName );
		a.appendChild(text);
	}
	else
	{
	...
	a.setAttribute( "href","service_add.jsp?parentid="+uniqueId);
	a.setAttribute( "title","add service");
	cell.appendChild( a );
	...
	a.setAttribute( "href","service_del.jsp?id="+uniqueId);
	a.setAttribute( "title","remove service");
	cell.appendChild( a );
	text=document.createTextNode(" -");
