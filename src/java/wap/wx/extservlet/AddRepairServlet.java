...
@WebServlet(name = "AddRepairServlet", urlPatterns = { "/extservlet/AddRepairServlet" })
public class AddRepairServlet extends ExtBaseServlet {

	String path = null;
	public void addRepair(HttpServletRequest request,
		...
		if (jsonObj != null) {
			...
			String username = jsonObj.get("username")==null?"无名":(String) jsonObj.get("username");
			...
			StringBuffer urlpost = new StringBuffer(
					ExtWxConfig.APIADDR+"/api/post_addRepairInfo.ashx");
			//Logger.getLogger(AddRepairServlet.class).debug("申请报修请求信息:" + urlpost);
			...
			StringBody btel = new StringBody(tel,ContentType.MULTIPART_FORM_DATA.withCharset("UTF-8"));
			StringBody breptype = new StringBody(reptype,ContentType.MULTIPART_FORM_DATA.withCharset("UTF-8"));
			StringBody busername = new StringBody(username,ContentType.MULTIPART_FORM_DATA.withCharset("UTF-8"));
			StringBody bsaywhat = new StringBody(saywhat,ContentType.MULTIPART_FORM_DATA.withCharset("UTF-8"));
			
			MultipartEntityBuilder builder = MultipartEntityBuilder.create();
			builder.setMode(HttpMultipartMode.STRICT);
			builder.addPart("ukey", bukey).addPart("tel", btel).addPart("type", breptype)
			.addPart("contacts", busername).addPart("content", bsaywhat);
			...
			Logger.getLogger(AddRepairServlet.class).debug("申请报修返回信息:" + jsonStr);
